...
import { defaultAuthParams } from "ra-core/dist/cjs/auth/useAuthProvider.js";

function ThreeColSimpleWithImage() {
  ...
  async function fetchCategory() {
    try {
      let res = await axios.get(`http://localhost:8000/categories/${name}`)
  ...
  async function fetchPosts() {
    try {
      let res = await axios.get(`http://localhost:8000/categories/posts/${name}`)
          ...
          {postList.map((post, index) => (
            <Column key={index}>
              <Card>
                ...
                <Category>{post.category1}</Category>
